...
class UDPHandler(socketserver.BaseRequestHandler):
    ...
    def processInvite(self):
        ...
        if len(destination) > 0:
            ...
            if destination in registrar:
                ...
                self.sendResponse("480 Dočasne nedostupne")
        else:
    ...
    def processAck(self):
        ...
        if len(destination) > 0:
            if destination in registrar:
                callID = self.getCallID()
                socket, claddr = self.getSocketInfo(destination)
                logging.info("Čas prijatia hovoru: " + time.strftime("%H:%M:%S", time.localtime()) + f", ID Hovoru:{callID}")
                self.data = self.addTopVia()
                data = self.removeRouteHeader()
                data.insert(1, recordroute)
                text = "\r\n".join(data).encode('utf-8')
    ...
    def processNonInvite(self):
        ...
        if rx_bye.search(request_uri):
            logging.info(f"Hovor ukončený, ID hovoru:{callID}, Čas ukončenia: " + time.strftime("%H:%M:%S", time.localtime()))
        if len(origin) == 0 or not origin in registrar:
            self.sendResponse("400 Zly Request")
            return
        destination = self.getDestination()
        if len(destination) > 0:
            if destination in registrar:
                ...
                self.sendResponse("406 Neda sa prijať")
        else:
            self.sendResponse("500 Interny server error")
